...
export default function Arts({ navigate }) {
  return (
    <PageLayout navigate={navigate} icon="🎨" title="Arts (FA — Faculty of Arts)" breadcrumb="Academics → Intermediate → Arts (FA)">
      <style>{`
      ...
      <div className="hero-banner" style={{background:"linear-gradient(135deg,#103d25 0%,#1a5c38 60%,#2a7a4e 100%)",borderRadius:"18px",padding:"44px 40px",marginBottom:"36px",position:"relative",overflow:"hidden",boxShadow:"0 12px 40px rgba(14,116,144,0.25)"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:"16px",marginBottom:"16px",flexWrap:"wrap"}}>
          ...
          <div>
            ...
            <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:"clamp(20px,3vw,28px)",color:"#fff"}}>Arts — FA (Faculty of Arts)</h2>
          </div>
        </div>
        <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:"15px",color:"rgba(255,255,255,0.8)",maxWidth:"620px",lineHeight:1.7}}>Explore humanities, languages, social sciences, and the arts. A pathway to Law, Political Science, English Literature, Journalism, and Civil Services.</p>
      </div>
      <div className="three-col" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"14px",marginBottom:"36px"}}>
        {[{icon:"📅",label:"Duration",val:"2 Years"},{icon:"🏫",label:"Classes",val:"Mon – Sat"},{icon:"⏰",label:"Timing",val:"8:00 AM – 2:00 PM"},{icon:"📋",label:"Board",val:"BISE Peshawar"}].map((s,i)=>(<div key={i} style={{background:"#fff",borderRadius:"12px",padding:"18px 14px",textAlign:"center",border:"1px solid #e9ecef",boxShadow:"0 2px 8px rgba(0,0,0,0.04)",animation:`fadeUp 0.4s ease ${i*0.07}s both`}}><div style={{fontSize:"26px",marginBottom:"6px"}}>{s.icon}</div><div style={{fontFamily:"'Playfair Display',serif",fontSize:"16px",color:"#103d25",fontWeight:700}}>{s.val}</div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:"11px",color:"#9ca3af",marginTop:"3px",textTransform:"uppercase",letterSpacing:"0.5px"}}>{s.label}</div></div>))}
      </div>
      <div className="info-card" style={{marginBottom:"28px"}}>
        ...
        <h3 style={{fontFamily:"'Playfair Display',serif",fontSize:"18px",color:"#103d25",marginBottom:"16px"}}>Curriculum — Compulsory & Elective</h3>
        <div className="two-col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 32px"}}>
          <div>
            ...
            {["English","Urdu","Islamic Education / Ethics","Pakistan Studies"].map((s,i)=>(<div key={i} className="subject-row"><span style={{width:"8px",height:"8px",borderRadius:"50%",background:"#1a5c38",flexShrink:0}}/><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:"14px",color:"#374151"}}>{s}</span></div>))}
          </div>
          <div>
            <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:"12px",color:"#103d25",fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",marginBottom:"10px"}}>Electives (Choose 3)</p>
            {["Economics","History","Civics","Geography","Sociology","Pashto / Persian"].map((s,i)=>(<div key={i} className="subject-row"><span style={{width:"8px",height:"8px",borderRadius:"50%",background:"#103d25",flexShrink:0}}/><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:"14px",color:"#374151"}}>{s}</span></div>))}
          </div>
        </div>
      </div>
      <div className="info-card" style={{marginBottom:"28px"}}>
        ...
        {[{label:"Admission Fee (one-time)",amount:"PKR 1,500"},{label:"Annual Tuition Fee",amount:"PKR 6,000"},{label:"Examination Fee",amount:"PKR 1,500"},{label:"Library Fee",amount:"PKR 500"},{label:"Sports & Misc Fee",amount:"PKR 800"},{label:"Total (Year 1 approx.)",amount:"PKR 10,300",bold:true}].map((f,i)=>(<div key={i} className="fee-row" style={{fontWeight:f.bold?"700":"400",color:f.bold?"#103d25":"#374151"}}><span>{f.label}</span><span style={{color:f.bold?"#103d25":"#6b7280",fontWeight:f.bold?"700":"500"}}>{f.amount}</span></div>))}
        <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:"12px",color:"#9ca3af",marginTop:"12px"}}>* Fees are subject to change. Confirm with administration.</p>
      </div>
      <div style={{marginBottom:"28px"}}>
        ...
        <div className="two-col" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"14px"}}>
          {[
  { name: "Prof. Ahsan Ali", subject: "English Literature", qual: "MA English" },
  { name: "Prof. Saad Khan", subject: "Urdu", qual: "MA Urdu" },
  { name: "Prof. Imran Malik", subject: "Pakistan Studies", qual: "MA History" },
  { name: "Prof. Naveed Ahmed", subject: "Economics", qual: "MA Economics" },
  { name: "Prof. Faizan Shah", subject: "Civics / Sociology", qual: "MA Political Science" },
  { name: "Prof. Daniyal Raza", subject: "History", qual: "MA History" }
].map((p,i)=>(<div key={i} className="prof-card" style={{animationDelay:`${i*0.07}s`}}><div style={{width:"52px",height:"52px",borderRadius:"50%",background:"linear-gradient(135deg,#103d25,#2a7a4e)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"22px",margin:"0 auto 12px"}}>👨‍🏫</div><div style={{fontFamily:"'Playfair Display',serif",fontSize:"15px",color:"#103d25",marginBottom:"4px"}}>{p.name}</div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:"12.5px",color:"#103d25",fontWeight:600,marginBottom:"3px"}}>{p.subject}</div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:"12px",color:"#9ca3af"}}>{p.qual}</div></div>))}
        </div>
      </div>
      <div className="two-col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"20px",marginBottom:"28px"}}>
        <div className="info-card"><div className="badge" style={{background:"rgba(14,116,144,0.08)",color:"#103d25",marginBottom:"14px"}}>✅ Eligibility</div>{["Passed Matric (SSC) with 45%+ marks","Any group in Matric acceptable","Age: 15–20 years","Domicile of KPK / Federal Area"].map((e,i)=>(<div key={i} style={{display:"flex",gap:"10px",marginBottom:"9px",alignItems:"flex-start"}}><span style={{color:"#103d25",fontWeight:700,fontSize:"14px",flexShrink:0}}>✓</span><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:"14px",color:"#374151",lineHeight:1.5}}>{e}</span></div>))}</div>
        <div className="info-card"><div className="badge" style={{background:"rgba(14,116,144,0.08)",color:"#103d25",marginBottom:"14px"}}>🚀 Career Paths</div>{["BS English Literature","BS Political Science","BS Journalism","LLB (Law)","CSS / PMS Civil Services"].map((c,i)=>(<div key={i} style={{display:"flex",gap:"10px",marginBottom:"9px",alignItems:"flex-start"}}><span style={{color:"#c9a84c",fontWeight:700,fontSize:"14px",flexShrink:0}}>→</span><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:"14px",color:"#374151",lineHeight:1.5}}>{c}</span></div>))}</div>
      </div>
